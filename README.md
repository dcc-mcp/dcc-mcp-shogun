...
its `Timeline` and `Offline` interfaces. It does not expose arbitrary Python or
HSL execution. Application controls not exposed by the SDK remain an explicit,
...
- query one marker trajectory value or an inclusive window of at most 2,000
  frames;
- inspect and explicitly change current frame, selected time ranges, and play
  range through the capability-gated `Timeline` interface;
- inspect a stable allowlist of processing settings and invoke reconstruct,
  auto-label, occlusion fixing, solve, or retarget through the official
  `Offline` interface;
...
closed unless `overwrite=true`, and public results omit full paths.
...
[Python SDK interface guide](https://vicon-help.atlassian.net/wiki/spaces/Shogun111/pages/13208701/Use%2BVicon%2BShogunPost%2BSDK%2Binterfaces)
...
- The scene Skill is read-only; the file Skill exposes only bounded
  import/save/export operations; processing never defaults to the full play
  range.
