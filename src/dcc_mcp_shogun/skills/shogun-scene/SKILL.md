...
metadata:
  dcc-mcp:
    ...
    search-hint: >-
      ...
      objects hierarchy transforms selection visibility opacity VDF animation data
    tools: tools.yaml
---
...
object-detail, selection, or display-property call. Those three controls are
non-destructive but can change the user's current view/selection. The Skill does
not expose object creation, removal, renaming, reparenting, raw attribute/channel
writes, or arbitrary HSL/Python. Use the separate `shogun-files` Skill for the
three explicitly bounded file mutations implemented through the official SDK.
